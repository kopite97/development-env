...
Home layout uses Dashboard v3: GET/PUT `/api/v3/dashboards/home` returns `initialized`, `layoutRevision`, ordered `placements` and server-owned Widget snapshots. An uninitialized Home stays empty until the user chooses the six-Widget initialization POST or an empty-layout PUT. Widget titles and selections are stored independently through Widget v1 (`/api/v1/widgets`); each Widget's typed data comes from `/api/v1/widgets/{id}/data`. Layout and Widget revisions are saved separately, and uncertain or partial writes retain the draft for explicit review.

The six supported Widget types keep the existing grid, frame, drag/arrow ordering, widths and editor presentation. Existing Widget types are immutable; removing a placement keeps the Widget available for reuse. Home URL filters are not sent as Widget data overrides, and deploy reports the server's `NOT_CONFIGURED` state. `devspace.layout.v1` and schema-2 Dashboard data remain preserved and unused by authenticated code.

Start Vite on port 4183, set `DASHBOARD_REUSE_SERVER=1`, and run `npx playwright test --config playwright.dashboard.config.ts` for the current v3 mock regression suite. Superseded v2 scenarios remain under `tests/dashboard/legacy-v2/` for historical reference. Run `node tests/real/run.mjs --dashboard=all` for disposable Widget/Dashboard and authentication acceptance. See [PLAN-0023](docs/plans/PLAN-0023-widget-dashboard-api-refactoring.md), [PLAN-0024](docs/plans/PLAN-0024-dashboard-widget-interactions.md) and their [validation evidence](docs/reviews/plan0024-dashboard-widget-interactions/readme.md).
