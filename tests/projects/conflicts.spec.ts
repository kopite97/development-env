...
test('failed conflict reload and repeated conflict retain the draft without automatic writes', async ({
  ...
  await page.getByLabel('Name', { exact: true }).fill('Retain draft');
  await page.getByRole('button', { name: 'Save Project' }).click();
  await expect(
    page.getByText('Latest Project could not be loaded. Retry reconciliation before editing.'),
  ).toBeVisible();
  await expect(page.getByLabel('Name', { exact: true })).toHaveValue('Retain draft');
  expect(writes).toBe(1);
  failRead = false;
  await page.getByRole('button', { name: 'Reload latest Project' }).click();
...
test('two browser tabs cannot silently overwrite each other', async ({ page, context }) => {
  ...
  let current = project(1),
    writes = 0;
  await context.route('**/api/v1/projects/' + id(1), (route) => {
  ...
  await page.getByLabel('Name', { exact: true }).fill('First tab');
  await second.getByLabel('Name', { exact: true }).fill('Second tab');
  await page.getByRole('button', { name: 'Save Project' }).click();
