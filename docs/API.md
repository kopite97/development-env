...
현재 구현은 React 프론트엔드 프로토타입입니다. 배치·프로젝트·태스크·일지는 localStorage로 관리하며 초기 예시 데이터를 제공합니다. 마일스톤·운영 상태는 예시 데이터입니다. 아래 API는 **구현 예정 계약**이며 현재 호출하거나 구현하지 않습니다. Java/Spring Boot 또는 C#/ASP.NET Core 어느 쪽에도 적용할 수 있습니다.
...
프로젝트 등록·편집·보관, 작업 생성·편집·휴지통, 일지 작성은 로컬 저장으로 구현했습니다. 아래 서버 API는 아직 구현하지 않았으며 나머지 작성·삭제 UI는 후속 기능입니다.
...
태스크 휴지통 연동에는 GET `/tasks?deleted=true`와 POST `/tasks/{id}/restore`를 추가합니다. DELETE는 복구 가능한 소프트 삭제로 처리하고 일반 목록·통계에서 제외합니다. 현재 로컬 프로젝트의 archived는 서버 계약의 status=archived로 매핑하며, 보관은 연결된 작업·일지를 삭제하지 않습니다.
