...
Category names are display data, not IDs. Normal business reads/writes are v2; Category/auth are v1. Use Dashboard schema 2 and remove response-only selectionState on writes. Preserve historical v1 omission-based bodies and endpoints; never load legacy providers to resolve Categories. Do not use old feature runner flags against a migrated backend: the runner rejects them. Its empty-test-DB Flyway override must never be copied into a populated workspace deployment. Re-run related feature browser suites when modifying Project serialization or invalidation. See [PLAN-0014](../plans/PLAN-0014-project-category-only-transition.md) for the acceptance matrix.
