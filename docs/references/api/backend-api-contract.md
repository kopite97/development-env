...
안정화 1~8번까지의 프론트엔드 구현을 기준으로 작성한 서버 설계 계약입니다. 서버는 아직 구현되지 않았으며 이 문서는 현재 동작하는 API 명세가 아닙니다. 구현 착수 시 현재 프론트엔드와의 차이를 다시 확인합니다. 현재 구조는 [아키텍처](../../architecture/overview.md), 실행 순서와 상태는 [계획 목록](../../plans/README.md), 남은 기능은 [안정화 계획](../../plans/frontend-stabilization.md)을 참고합니다.
